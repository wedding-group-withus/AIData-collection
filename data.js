...
window.CompetitorsData = [
  ...
  },
  {
    ...
    baseRentalFeeRange: { min: 4000000, max: 6000000 },
    baseMealCostRange: { min: 70000, max: 78000 },
    location: "안양 동안구 (범계역 7번 출구 도보 5분, 동안구청 맞은편)",
    strengths: "4호선 범계역 초역세권으로 대중교통 접근성이 우수하고 하객 방문이 수월함, 단독홀로 구성되어 비교적 여유롭고 하객이 겹치지 않는 프라이빗함, 1층에 웨딩홀과 연회장이 함께 위치해 계단/엘리베이터 이동이 없는 편리한 원스톱 동선, 맛있는 뷔페 퀄리티(1인당 개별 스테이크 별도 제공)로 만족도 높음",
    weaknesses: [
      "하우스 웨딩 특화 아기자기한 컨셉으로 대형 예식장에 비해 홀 규모가 다소 아담하고 천장고가 높지 않음",
      "단독홀이나 1층 공간의 제약으로 예식 전후 앞뒤 타임 하객이 일시 집중될 경우 로비와 복도가 다소 혼잡하고 정체될 수 있음",
      "자체 주차장(지하/지상) 보유로 주차는 가능하나 입출차 진입로 각도가 다소 좁아 복잡한 시간대에 하객 차량 대기가 발생함"
    ],
    reviews: [],
    pricingModel: {
      peakMonths: [4, 5, 9, 10, 11],
      ...
      timeSlots: {
        golden: { rentalMultiplier: 1.30, mealMultiplier: 1.05, minGuarantee: 250 },
        regular: { rentalMultiplier: 1.00, mealMultiplier: 1.00, minGuarantee: 200 },
        late: { rentalMultiplier: 0.60, mealMultiplier: 0.92, minGuarantee: 150 }
      }
    },
    overrides: {}
  }
...
window.getStrategy = (competitorId, month, timeSlot, year = 2026) => {
  ...
  switch (competitorId) {
    ...
    case "villadegd":
      ...
      break;
    case "patiobella":
      adviseMent = "범계역 역세권과 하우스 웨딩 분위기를 존중하되, 하객이 많을 때의 복도 혼잡성 및 웅장한 극장식 층고 대비 아담한 층고 제약을 대조하여 위더스의 압도적 공간 밸류를 강조하십시오.";
      detailReason = `${competitor.name}은 범계역 역세권에 단독홀 및 편리한 1층 동선(홀-연회장 동층)이 메리트이지만, 하객이 200명 이상 유입될 시 로비와 복도가 비좁아질 수 있고 층고가 낮아 하객 답답함이 유발될 수 있습니다. 위더스는 단독 건물 전체를 사용하여 층별로 로비와 예식홀이 완전히 독립 분리되어 혼잡을 원천 차단하며, 층고가 웅장하고 버진로드가 25m로 길어 예식의 격조가 다릅니다.`;
      counterScript = [
        { role: "신랑신부", text: "파티오벨라는 범계역이랑 되게 가깝고, 홀이랑 연회장이 같은 층이라 하객분들 동선이 진짜 편할 것 같아요." },
        { role: "위더스 상담원", text: "네, 맞습니다. 4호선 범계역 역세권이라 하객분들 찾아오시기 매우 수월하고, 같은 층에 예식홀과 식사 공간이 다 연결되어 동선이 심플한 것은 큰 장점입니다. 다만 단독홀임에도 1층이라는 공간의 특성상 복도와 축의대 로비가 비교적 콤팩트하게 구성되어 있습니다. 이 때문에 앞 타임 하객분들과 뒤 타임 하객분들이 축의대에서 겹치게 될 때 다소 혼잡함을 느끼실 수 있습니다. 또한, 하우스 웨딩 느낌으로 아늑하게 설계된 만큼 홀의 천장고가 다소 낮아 하객이 많을 시 개방감이 아쉬울 수 있습니다. 저희 위더스 안양점은 웅장하고 높은 극장식 층고와 안양 최대 25m 직선 버진로드를 자랑하며, 건물의 층별로 홀과 로비를 오롯이 분리 사용하여 혼선이 없습니다. 귀빈들을 쾌적하고 격조 있게 모시는 데 최적화된 위더스만의 스페셜 혜택도 함께 검토해 보시길 권해드립니다." }
      ];
      break;
  }
